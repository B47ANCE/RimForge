...
Assert-NotMatch $viewCode 'DependencyNodes\.ToDictionary' 'Outline still uses duplicate-unsafe ToDictionary construction.'
Assert-Match $viewCode 'nodes\.ContainsKey\(key\)' 'Outline does not tolerate duplicate graph package identifiers.'
Assert-Match $viewCode 'string\.IsNullOrWhiteSpace\(edge\.SourceId\)' 'Outline does not filter malformed relationship identifiers.'
Assert-Match $viewCode 'nodes\.ContainsKey\(edge\.SourceId\).*nodes\.ContainsKey\(edge\.TargetId\)' 'Outline does not filter relationships to missing nodes.'
